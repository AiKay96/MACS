...
public class Piece {
	...
	*/
	...
	public Piece computeNextRotation() {
		...
		int minimalX = 0;
		//(x,y) >> (-y, x)
		//rotating 90° on xy plane
		for(int i = 0; i < body.length; i++){
			newBody[i] = new TPoint(body[i].y * (-1), body[i].x);
			minimalX = Math.min(minimalX, newBody[i].x);
		}
		minimalX *= -1;
		//moving figure in first quadrant
		for(int i = 0; i < body.length; i++)
			newBody[i] = new TPoint(newBody[i].x + minimalX, newBody[i].y);
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		Piece curr = root;
		while(true){
			Piece currNext = curr.computeNextRotation();
