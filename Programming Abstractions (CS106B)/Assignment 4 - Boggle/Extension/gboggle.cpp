...
const string BLOCK_COLOR = "#D60209";
...
static void drawRedCube(int row, int col, char ch, bool invert);
...
static void drawHelps(int a, int b, int c);
static void eraseOldHelps(int a, int b, int c);
 ...
 * Function: blockCube
 * -----------------------
 * Exported function used to turn off and on block for cubes.  This
 * function is intended to block a cube which is currently being used
 * to form a word.
 */
void blockCube(int row, int col, bool flag) {
   if (row < 0 || row >= gState.numRows || col < 0 || col >= gState.numCols) {
      error("labelCube called with invalid row, col arguments.");
   }
   drawRedCube(row, col, gState.letters[row][col], flag);
}
/*
 ...
 * Function: drawHelps
 * ----------------------
 * Draws new helps numbers for user.
 */
void showHelps(int a, int b, int c){
	eraseOldHelps(a, b, c);
	drawHelps(a, b , c);
}
/*
 * Function: drawHelps
 * ----------------------
 * Draws the helps numbers for user.
 */
static void drawHelps(int a, int b, int c) {
	string s = "SWAP: " + integerToString(a) + "  " + "BLOCK: " + integerToString(b) + "  " + "DICTATION: " + integerToString(c);
    GLabel label(s);
    label.setFont(SCORE_FONT + "-" + integerToString(SCORE_FONT_SIZE - 3));
    label.setColor(LABEL_COLOR);
    label.setLocation(15, 15);
    gwp->draw(label);
}
/*
 * Function: eraseOldHelps
 * ----------------------
 * Erase the helps numbers for user.
 */
static void eraseOldHelps(int a, int b, int c) {
	string s = "SWAP: " + integerToString(a) + "  " + "BLOCK: " + integerToString(b) + "  " + "DICTATION: " + integerToString(c);
   GLabel label(s);
   ...
   fillBox(15, 0, width, SCORE_FONT_SIZE - 3, "White");
}
/*
