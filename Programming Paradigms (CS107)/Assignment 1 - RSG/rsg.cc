...
void recursion(string& s, map<string, Definition>& grammar);
...
void recursion(string& s, map<string, Definition>& grammar){
  Production product = grammar[s].getRandomProduction();
  vector<string>::iterator it = product.begin();
  while(it != product.end()){
    string str = *it;
    if(str[0] == '<')
      recursion(str, grammar);
    else
      cout << str << " ";
    it++;
  }
