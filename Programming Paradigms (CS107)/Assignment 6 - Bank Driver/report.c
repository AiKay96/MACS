...
{
  sem_wait(&bank->work_done_lock);
  bank->work_done++;

  if(bank->work_done < numWorkers){
    sem_post(&bank->work_done_lock);
    sem_wait(&bank->next_day_lock);
    sem_post(&bank->wait_lock);
    return 0;
  }else{
    Report *rpt = bank->report;
    ...
    if (rpt->numReports >= MAX_NUM_REPORTS) {
      for(int i = 1; i < numWorkers; i++)
        sem_post(&bank->next_day_lock);
      bank->work_done = 0;

      for(int i = 1; i < numWorkers; i++)
        sem_wait(&bank->wait_lock);
