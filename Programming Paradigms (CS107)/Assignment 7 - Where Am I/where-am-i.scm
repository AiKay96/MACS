...
(define (partition pivot num-list compare)
  (if (null? num-list) '(() ())
      (let ((split-of-rest (partition pivot (cdr num-list) compare)))
	(if (compare (car num-list) pivot)
...
(define (quicksort num-list compare)
  (if (<= (length num-list) 1) num-list
      (let ((split (partition (car num-list) (cdr num-list) compare)))
	(append (quicksort (car split) compare) 
		(list (car num-list)) 
		(quicksort (cadr split) compare)))))
...
(define (intersection-points-line circleList)
  (apply append (map (lambda (i) (intersect (car circleList) i)) (cdr circleList))))
(define (intersection-points circleList)
  (if (<= (length circleList) 1) '()
      (append (intersection-points-line circleList) (intersection-points (cdr circleList)))))
(define (countProduct distList)
  (if (null? distList) 1
      (* (car distList) (countProduct (cdr distList)))))
(define (myDist point1 point2)
  (if (equal? point1 point2) 1
      (dist point1 point2)))
(define (countDists point pointList)
   (map (lambda (i) (myDist point i)) pointList))
(define (distance-product point pointList)
  (countProduct(countDists point pointList)))
(define (product-point point pointList)
  (cons (distance-product point (reverse pointList)) (list point)))
(define (rate-points pointList)
  (map (lambda (i) (product-point i pointList)) pointList))
(define (compare elem1 elem2)
  (if (< (car elem1) (car elem2)) #t
      #f))
(define (sort-points list)
  (quicksort list compare))
(define (deleteHalf list)
  (map (lambda (i) (cadr i)) list))
(define (clumped-points list)
  (deleteHalf (prefix-of-list (sort-points (rate-points list)) (quotient (length list) 2))))
(define (countX list)
  (if (null? list) 0
      (+ (car (car list)) (countX (cdr list)))))
(define (countY list)
  (if (null? list) 0
      (+ (cadr (car list)) (countY (cdr list)))))
(define (average-point list)
  (let ((x (/ (countX (reverse list)) (length list))) (y (/ (countY (reverse list)) (length list))))
  (cons (distance-product (append (cons x '()) (cons y '())) (reverse list)) (cons (append (cons x '()) (cons y '())) '()))))
(define (best-estimate circleList)
  (average-point (clumped-points (intersection-points circleList))))
(define (where-am-i distanceList starList)
 (sort-points (map (lambda (i) (best-estimate i)) (all-guesses distanceList starList))))
